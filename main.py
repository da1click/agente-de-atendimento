...
async def chatwoot_webhook(request: Request):
    ...
    for msg in messages:
        ...
        # IA nunca deve enviar mensagem em conversas desqualificadas
        if "desqualificado" in conv_labels:
            logger.info(f"[ia] Bloqueado por etiqueta 'desqualificado' — conv={conversation_id} account={account_id}")
            continue

        # Conta 19: IA desativada
        if account_id == 19:
            logger.info(f"[ia] Conta 19 — IA desativada (conv={conversation_id})")
            continue
