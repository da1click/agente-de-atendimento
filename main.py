...
from db import (
    inserir_tracking_event, tracking_event_existe,
    listar_tracking_events, contar_tracking_por_origem,
)
    ...
    for msg in messages:
        ...
        if not ia_ativa and ia_config_ok and ia_agent_id is not None and inbox_permitido and conversation_id:
            if assignee_id is None or assignee_id != ia_agent_id:
                ...
                last_rc_json = None
                last_rc_assignee_id = assignee_id  # guardar o que havia antes das tentativas
                    ...
                    try:
                        ...
                        async with httpx.AsyncClient(timeout=10) as hc:
                            ...
                            if resp_rc.is_success:
                                ...
                                last_rc_assignee_id = rc_assignee_id
                ...
                if not ia_ativa and last_rc_json is not None:
                    msgs_count = last_rc_json.get("messages_count", 99)
                    assign_url = f"{chatwoot_url_rc}/api/v1/accounts/{account_id}/conversations/{conversation_id}/assignments"

                    # Caso 1: assignee ainda None após as tentativas → automação ainda não rodou.
                    # Inbox configurada = conversa destinada à IA. Forçar para todas as contas.
                    if last_rc_assignee_id is None and msgs_count <= 2:
                        try:
                            async with httpx.AsyncClient(timeout=10) as hc:
                                ...
                                if r.is_success:
                                    ...
                                    ia_ativa = True
                                    logger.info(f"[race-condition] IA força-atribuída (assignee null, inbox configurada) — conv={conversation_id} account={account_id}")
                                else:
                                    logger.warning(f"[race-condition] Falha ao força-atribuir IA (null): {r.status_code} — conv={conversation_id}")
                        except Exception as e:
...
# ══════════════════════════════════════════════════════════════
# TRACKEAMENTO — ORIGEM DA CONVERSA
# ══════════════════════════════════════════════════════════════

def _extrair(texto: str, rotulo: str) -> str:
    """Extrai 'rotulo:' (no início de linha) seguido do valor na MESMA linha."""
    pat = rf"(?:^|\n)\s*{re.escape(rotulo)}[ \t]*:[ \t]*([^\r\n]*)"
    m = re.search(pat, texto, re.IGNORECASE)
    return (m.group(1).strip() if m else "")


def classificar_origem(content: str) -> dict:
    """Classifica a primeira mensagem em: anuncio_meta, site ou direto.
    Retorna dict com 'origem' + campos específicos preenchidos."""
    texto = (content or "").strip()
    base = {"origem": "direto", "content_raw": texto}
    ...
    # ── Padrão 1: Anúncio Meta (Instagram / Facebook)
    if (
        ...
        base["origem"] = "anuncio_meta"
        base["anuncio_texto"] = _extrair(limpo, "Texto Anuncio") or _extrair(limpo, "Texto Anúncio")
        base["anuncio_url"] = _extrair(limpo, "URL")
        base["anuncio_source_id"] = _extrair(limpo, "SourceID")
        base["anuncio_fonte"] = _extrair(limpo, "Fonte").upper() or None
        base["anuncio_ref_id"] = _extrair(limpo, "ID")
        base["anuncio_tipo"] = _extrair(limpo, "Tipo")
        base["anuncio_resposta"] = _extrair(limpo, "Resposta do contato")
        return base

    # ── Padrão 2: Formulário de site
    if re.search(r"Contato\s+via\s+Site", limpo, re.IGNORECASE):
        base["origem"] = "site"
        base["site_nome"] = _extrair(limpo, "Nome")
        base["site_email"] = _extrair(limpo, "E-mail") or _extrair(limpo, "Email")
        base["site_telefone"] = _extrair(limpo, "Telefone")
        base["site_assunto"] = _extrair(limpo, "Assunto")
        m = re.search(r"(?:^|\n)\s*Mensagem[ \t]*:[ \t]*(.*)", limpo, re.IGNORECASE | re.DOTALL)
        base["site_mensagem"] = (m.group(1).strip() if m else "")[:2000]
        return base

    # ── Padrão 3: Mensagem direta (qualquer outra coisa)
    base["direto_frase"] = texto[:500]
    return base


@app.post("/webhook/tracking")
async def webhook_tracking(request: Request):
    """Webhook exclusivo de tracking. Recebe apenas 'message_created' do Chatwoot
    e grava a primeira mensagem incoming de cada conversa, classificando a origem."""
    payload = await request.json()
    event = payload.get("event", "")
    ...
    conversation_id = payload.get("conversation_id") or conversation.get("id")
    inbox_id = payload.get("inbox_id") or payload.get("inbox", {}).get("id")
    inbox_name = (payload.get("inbox") or {}).get("name") or ""
    message_id = payload.get("id")
    message_type = payload.get("message_type")
    content = payload.get("content") or ""

    # Só nos interessa mensagem incoming (cliente)
    # Chatwoot: message_type == "incoming" ou 0
    if message_type not in ("incoming", 0):
        return {"status": "ignorado", "motivo": "nao_incoming"}

    if not account_id or not conversation_id:
        return {"status": "ignorado", "motivo": "faltam_ids"}

    # Idempotência — uma linha por conversa
    if tracking_event_existe(account_id, conversation_id):
        return {"status": "ja_registrado", "conversation_id": conversation_id}
    ...
    dados.update(classificar_origem(content))

    inserir_tracking_event(dados)
    logger.info(
        f"[tracking] account={account_id} conv={conversation_id} "
        f"origem={dados.get('origem')} inbox={inbox_id}"
    )
    return {"status": "ok", "origem": dados.get("origem")}


TRACKING_WEBHOOK_URL = "https://api.advbrasil.ai/webhook/tracking"


async def _chatwoot_webhooks_da_conta(base_url: str, token: str, account_id: int) -> list[dict]:
...
async def _registrar_webhook_tracking(account_id: int) -> dict:
    """Idempotente: checa se já existe um webhook de tracking e cria se não houver."""
    config = carregar_config_cliente(account_id)
    if not config:
        return {"ok": False, "motivo": "config_nao_encontrada"}

    base_url = (config.get("chatwoot_url") or "").rstrip("/")
    token = config.get("chatwoot_token", "")
    if not base_url or not token:
        return {"ok": False, "motivo": "sem_chatwoot"}

    existentes = await _chatwoot_webhooks_da_conta(base_url, token, account_id)
    for wh in existentes:
        if (wh.get("url") or "").rstrip("/") == TRACKING_WEBHOOK_URL.rstrip("/"):
            return {"ok": True, "status": "ja_existe", "webhook_id": wh.get("id")}

    async with httpx.AsyncClient(timeout=20) as client:
        r = await client.post(
            f"{base_url}/api/v1/accounts/{account_id}/webhooks",
            headers={"api_access_token": token, "Content-Type": "application/json"},
            json={"url": TRACKING_WEBHOOK_URL, "subscriptions": ["message_created"]},
        )
        if r.status_code in (200, 201):
            return {"ok": True, "status": "criado", "webhook_id": r.json().get("id")}
        return {"ok": False, "motivo": f"http_{r.status_code}", "body": r.text[:300]}


@app.post("/api/tracking/{account_id}/registrar-webhook")
async def api_tracking_registrar_webhook(account_id: int, user=Depends(get_current_user)):
    return await _registrar_webhook_tracking(account_id)


@app.post("/api/tracking/registrar-todos")
async def api_tracking_registrar_todos(user=Depends(require_super_admin)):
    """Registra o webhook de tracking em todas as contas ativas que têm Chatwoot configurado."""
    from db import get_db
    db = get_db()
    configs = (
    ...
    for cfg in configs.data or []:
        ...
        if not aid:
            continue
        r = await _registrar_webhook_tracking(aid)
        r["account_id"] = aid
        r["nome"] = cfg.get("nome")
        resultados.append(r)
...
@app.get("/api/tracking/{account_id}")
async def api_tracking_listar(account_id: int, origem: str | None = None,
                              inbox_id: int | None = None, limite: int = 100,
                              offset: int = 0, user=Depends(get_current_user)):
    return {
        "resumo": contar_tracking_por_origem(account_id),
        "eventos": listar_tracking_events(account_id, origem, inbox_id, limite, offset),
    }


@app.get("/api/tracking/{account_id}/resumo")
async def api_tracking_resumo(account_id: int, user=Depends(get_current_user)):
    return contar_tracking_por_origem(account_id)


@app.get("/api/tracking/{account_id}/webhook-status")
async def api_tracking_webhook_status(account_id: int, user=Depends(get_current_user)):
    """Verifica se o webhook de tracking já está registrado na conta Chatwoot."""
    config = carregar_config_cliente(account_id)
    if not config:
        return {"registrado": False, "motivo": "config_nao_encontrada"}
    base_url = (config.get("chatwoot_url") or "").rstrip("/")
    token = config.get("chatwoot_token", "")
    if not base_url or not token:
        return {"registrado": False, "motivo": "sem_chatwoot"}
    existentes = await _chatwoot_webhooks_da_conta(base_url, token, account_id)
    for wh in existentes:
        if (wh.get("url") or "").rstrip("/") == TRACKING_WEBHOOK_URL.rstrip("/"):
            return {"registrado": True, "webhook_id": wh.get("id"),
                    "subscriptions": wh.get("subscriptions", [])}
